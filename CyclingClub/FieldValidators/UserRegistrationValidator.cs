...
namespace CyclingClub.FieldValidators
{
    public class UserRegistrationValidator:IFieldValidator
    {
        ...
        FieldValidatorDel _fieldVlaidatorDel = null;


        RequiredValidDel _requiredValidDel = null;
        RequiredLengthValidDel _requiredLenthValidDel = null;
        ...
        public FieldValidatorDel ValidatorDel => _fieldVlaidatorDel;
        ...
        {
            _fieldVlaidatorDel = new FieldValidatorDel(ValidField);
            _emailExistsDel = new EmailExistsDel(_register.EmailExists);

            _requiredValidDel = CommonFieldValidatorFunction.RequiredFiledValidDel;
            _requiredLenthValidDel = CommonFieldValidatorFunction.RequiredLengthValidDel;
            _dateValidDel = CommonFieldValidatorFunction.DateValidDel;
        ...
        private bool ValidField(int fieldIndex, string fieldValue, string[] fieldArray, out string fieldInvalidMessage)
        {
            ...
            switch(userRegistrationField)
            {
                ...
                case FieldConstants.UserRegistrationField.AddressSecondLine:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    break;
                case FieldConstants.UserRegistrationField.AdressCity:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    break;
